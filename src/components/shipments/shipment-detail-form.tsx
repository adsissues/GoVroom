...
export default function ShipmentDetailForm({
  ...
  const syncPalletBagRHFValues = useCallback((isPalletMode: boolean) => {
    ...
    if (isPalletMode) {
      ...
      // If numBags is 0 when switching to bag mode, default to 0 (user will then input)
       // if (getValues('numBags') === 0) { // This part can be removed if we just want it to be 0 for user input
       //    setValue('numBags', 0, { shouldValidate: false });
       // }
    }
  }, [setValue, getValues]);
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      if (detail) {
        ...
        // setTimeout(() => syncPalletBagRHFValues(true), 0); // Defer RHF value sync
      }
    }
  ...
  useEffect(() => {
    // This effect ensures RHF values (pallets/bags) are synced after mode change
    ...
    if (showPalletInputMode) { // Pallet mode
        ...
        if (numBagsWatched > 0) { // Bags have a count
            ...
            if (detail && detail.numPallets === 0 && detail.numBags === 0 && typeof detail.tareWeight === 'number') {
                ...
                // For new items, or items transitioning to "0 bags" state in bag mode
                newTareWeight = TARE_WEIGHT_DEFAULT; // Default to 25.7
            }
        }
    }
